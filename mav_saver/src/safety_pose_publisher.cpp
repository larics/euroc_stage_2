...
SafetyPosePublisher::SafetyPosePublisher(ros::NodeHandle& nh,
                                         ros::NodeHandle& private_nh)
    : take_control_(false), serial_port_open_(false) {
  ...
  private_nh.param("measurement_divisor", measurement_divisor_,
                   kDefaultMeasurementDivisor);
  SetupSerialPort();
}

SafetyPosePublisher::SafetyPosePublisher(int baudrate, std::string port,
                                         int measurement_divisor)
    : measurement_divisor_(measurement_divisor),
      take_control_(false),
...
bool SafetyPosePublisher::SetupSerialPort() {
  ...
  if (!serial_port->connect(port_, baudrate_)) {
    ROS_ERROR_STREAM("Could not connect to port " << port_ << " with baudrate "
                                                  << baudrate_);
    return false;
  }
...
void SafetyPosePublisher::SetPose(const Eigen::Vector3d& p_W_I,
  ...
  ++pose_counter_;
  if (pose_counter_ % measurement_divisor_ != 0) return;
  ...
  const unsigned int flags = EXT_POS_X_ACTIVE | EXT_POS_Y_ACTIVE |
                             EXT_POS_Z_ACTIVE | EXT_QUAT_YAW_ACTIVE;
  ...
  ethzasl_mav_interface::helper::rosGeographicPoseToAsctec(p_W_I, q_W_I,
                                                           &pos_asc, &q_asc);

  ROS_INFO_ONCE("sending pose ");
  ROS_INFO_STREAM_THROTTLE(
      2, "takeover: " << take_control_ << " x: " << pos_asc.x()
                      << " y: " << pos_asc.y() << " z: " << pos_asc.z());
...
unsigned short SafetyPosePublisher::CrcUpdate(unsigned short crc,
                                              unsigned char data) {
  data ^= (crc & 0xff);
  data ^= data << 4;
...
unsigned short SafetyPosePublisher::UpdateCrc16(unsigned short crc,
                                                const void* data,
                                                unsigned short cnt) {
  int i = 0;
  ...
  for (i = 0; i < cnt; i++) crcNew = CrcUpdate(crcNew, chrData[i]);
