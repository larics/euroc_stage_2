...
Uart::Uart() : fd_rx_(-1), fd_tx_(-1), ok_(true) {}
...
void Uart::closePort() {
  ...
  ::close(fd_rx_);
  if (port_rx_name_ != port_tx_name_) ::close(fd_tx_);
}
...
bool Uart::connect(const std::string& port_rx, const std::string& port_tx,
                   int baudrate) {
  bool success = false;
  ...
  // write packet that configures autobaud. LSB needs to be 1, LSB+1 needs to be
  // 0
  //  uint8_t buf = 'a';
  ...
  *serial_port = openPort(port);
  ...
  if (*serial_port == -1) {
    ROS_ERROR_STREAM("Could not open port "
                     << port);  // TODO(millanea): replace!
    return false;
  }

  if (!setupPort(*serial_port, &baudrate)) return false;

  ROS_INFO_STREAM("Opened serial port "
                  << port << " with baudrate "
                  << baudrate);  // TODO(millanea): replace!
  return true;
}

bool Uart::setupPort(int port_fd, int* baudrate, int data_bits, int stop_bits,
                     bool parity, bool hardware_control) {
  // Finding valid baudrate closest to requested baudrate
  int baudrates[] = {9600,   14400,  19200,  38400, 57600,
  ...
  if (best_baudrate != *baudrate)
    ROS_ERROR_STREAM(
        "Unsupported baudrate, choosing closest supported baudrate: "
        << best_baudrate);
  ...
  *baudrate = best_baudrate;
  ...
  if (!isatty(port_fd)) {
    ROS_ERROR_STREAM("File descriptor " << port_fd
                                        << " is NOT a serial port\n");
    return false;
  }
  if (tcgetattr(port_fd, &config) < 0) {
    ROS_ERROR_STREAM("Could not read configuration of fd "
                     << port_fd << ". Reason: " << strerror(errno));
    return false;
  }
  ...
  // Ignore CD line. Needed for OSX users (http://stackoverflow.com/a/24374870)
  // but be done in linux as well.
  ...
  switch (best_baudrate) {
    case 1200:
      success = (cfsetispeed(&config, B1200) == 0 &&
                 cfsetospeed(&config, B1200) == 0);
      break;
    case 1800:
      success = (cfsetispeed(&config, B1800) == 0 &&
                 cfsetospeed(&config, B1800) == 0);
      break;
    case 9600:
      success = (cfsetispeed(&config, B9600) == 0 &&
                 cfsetospeed(&config, B9600) == 0);
      break;
    case 19200:
      success = (cfsetispeed(&config, B19200) == 0 &&
                 cfsetospeed(&config, B19200) == 0);
      break;
    case 38400:
      success = (cfsetispeed(&config, B38400) == 0 &&
                 cfsetospeed(&config, B38400) == 0);
      break;
    case 57600:
      success = (cfsetispeed(&config, B57600) == 0 &&
                 cfsetospeed(&config, B57600) == 0);
      break;
    case 115200:
      success = (cfsetispeed(&config, B115200) == 0 &&
    ...
    case 460800:
      success = (cfsetispeed(&config, B460800) == 0 &&
                 cfsetospeed(&config, B460800) == 0);
      break;
    case 921600:
      success = (cfsetispeed(&config, B921600) == 0 &&
                 cfsetospeed(&config, B921600) == 0);
      break;
    default:
      ROS_ERROR_STREAM("" << best_baudrate
                          << " does not match any baudrate available.");
      return false;
  ...
  if (!success) {
    ROS_ERROR_STREAM("Baudrate "
                     << best_baudrate
                     << " could not be set. Reason: " << strerror(errno));
    return false;
  }

  // Apply the configuration.
  if (tcsetattr(port_fd, TCSAFLUSH, &config) < 0) {
    ROS_ERROR_STREAM("Could not set configuration of fd "
                     << port_fd << "Reason: " << strerror(errno));
    return false;
  }
...
int Uart::readBuffer(uint8_t* data, int size, int timeout_ms) {
  ...
  if (res == 1) {
    ...
    // ROS_INFO_STREAM("No data read within the last " << timeout_ms / 1000.0f
    // << "s");
    return 0;
  } else {
    ROS_ERROR_STREAM("read error:" << res << " " << errno << " "
                                   << strerror(errno));
    return -1;
  }
}

WriteQueue::WriteQueue() : fd_(-1), pos_(0), shutdown_(true) {}
...
void WriteQueue::writeThread() {
  ...
  while (!shutdown_) {
    ...
    while (pos_ == 0) {
      // Wait until signaled that new data in buffer. Periodically check for
      // shutdown
    ...
    if (res == -1)
      ROS_ERROR_STREAM("could not write "
                       << pos_ << " bytes, reason: " << strerror(errno));
...
int WriteQueue::writeBuffer(uint8_t* data, int size) {
  ...
  // Usually we get whole data packets, so it wouldn't make sense to truncate
  // the data here
  if (size > WRITE_BUFFER_SIZE - pos_) {
    ROS_ERROR_STREAM(
        "Send buffer full: only "
        << WRITE_BUFFER_SIZE - pos_ << " bytes of " << WRITE_BUFFER_SIZE
        << " bytes available, while trying to send " << size << "bytes.");
    return -1;
  }
