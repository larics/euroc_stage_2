...
TEST(kill_switch_library, test)
{
  ...
  EXPECT_FALSE( kill_switch.getKillStatus() ) << "Initial: Switch triggered when it should not be" ;

  // Triggering the switch and check
  switch_emulator.trigger();
  std::this_thread::sleep_for(std::chrono::milliseconds(200)) ;
  ROS_INFO("Trigger check");
  EXPECT_TRUE( kill_switch.getKillStatus() ) << "Post-Trigger: Switch not triggered when it should be" ;

  // Untriggering and checking it still is killed
  switch_emulator.untrigger();
  std::this_thread::sleep_for(std::chrono::milliseconds(200)) ;
  ROS_INFO("Untrigger check");
  EXPECT_TRUE( kill_switch.getKillStatus() ) << "Post-Untrigger: Switch not triggered when it should be" ;
  ...
  EXPECT_FALSE( kill_switch.getKillStatus() ) << "Post-reset: Switch triggered when it should not be" ;
