...
"""
import ConfigParser
from argparse import _SubParsersAction, _StoreAction, _StoreConstAction, _AppendAction
import argparse
...
class ArgParserWalker(object):
    ...
    def process_parser_action(self, action, is_store_const=False, is_list=False):
        raise NotImplementedError()
        ...
        try:
            ...
            for action in arg_parser._actions:
                if isinstance(action, _StoreAction):
                    self.process_parser_action(action, is_list = (hasattr(action, 'nargs') and action.nargs == '*' or action.nargs == '+' or action.nargs > 1))
                    ...
                    self.process_parser_action(action, is_list=True)
...
class GenerateConfig(ArgParserWalker):
    ...
    def process_parser_action(self, action, is_store_const=False, is_list=False):
        ...
        if self._only_non_defaults and action_value == (action.default if not hasattr(action, 'orgDefault') else action.orgDefault) :
            action_value = None
        if action_value is not None:
            if is_store_const:
                ...
                self._contents.append(u'{action_name}: {default_value}'.format(
                   ...
                   default_value= (",".join([str(v) for v in action_value]) if isinstance(action_value, list) else action_value),  # hope it prints as wanted...
                ))
...
class ReadConfig(ArgParserWalker):
    ...
    def process_parser_action(self, action, is_store_const=False, is_list=False):
        for option_string in action.option_strings if action.option_strings else [action.dest]:
            option_string = _convert_option_string(option_string)
            if self.config_parser.has_option(self.current_section,
                                             option_string):
                orgDefault = action.default;
                if is_store_const:
                    action.default = action.const
                    action.orgDefault = orgDefault;
                else:
                    ...
                    try:
                        if is_list: 
                            action.default = [type_transformer(v.strip()) for v in value.split(',')];
                        else:
