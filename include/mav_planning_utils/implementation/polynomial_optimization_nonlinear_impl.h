...
#ifndef POLYNOMIAL_OPTIMIZATION_NONLINEAR_IMPL_H_
#define POLYNOMIAL_OPTIMIZATION_NONLINEAR_IMPL_H_
...
bool PolynomialOptimizationNonLinear<_N>::setupFromVertices(
  ...
  opt_.reset(new nlopt::opt(optimization_parameters_.algorithm,
                            n_optimization_parameters));
  opt_->set_ftol_rel(optimization_parameters_.f_rel);
  opt_->set_ftol_abs(optimization_parameters_.f_abs);
  opt_->set_xtol_rel(optimization_parameters_.x_rel);
  opt_->set_xtol_abs(optimization_parameters_.x_abs);
  opt_->set_maxeval(optimization_parameters_.max_iterations);
...
int PolynomialOptimizationNonLinear<_N>::optimizeTime() {
  ...
  for (double t : segment_times) upper_bounds.push_back(t * 2.0);

  try {
    opt_->set_initial_step(initial_step);
    opt_->set_upper_bounds(upper_bounds);
    opt_->set_lower_bounds(0.1);
    opt_->set_min_objective(
  ...
  try {
    result = opt_->optimize(segment_times, final_cost);
  } catch (std::exception& e) {
...
int PolynomialOptimizationNonLinear<_N>::optimizeTimeAndFreeConstraints() {
  ...
  try {
    ...
    opt_->set_upper_bounds(upper_bounds);
    opt_->set_min_objective(&PolynomialOptimizationNonLinear<
  ...
  try {
    ...
    result = opt_->optimize(initial_solution, final_cost);
    timer_solve.Stop();
  } catch (std::exception& e) {
    LOG(ERROR) << "error while running nlopt: " << e.what() << std::endl;
    return nlopt::FAILURE;
...
#endif /* POLYNOMIAL_OPTIMIZATION_NONLINEAR_IMPL_H_ */
